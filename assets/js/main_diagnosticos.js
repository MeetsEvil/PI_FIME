...
document.addEventListener("DOMContentLoaded", () => {
    ...
    submitBtn.addEventListener("click", async (e) => {
        ...
        if (valid && form.checkValidity()) {
            await submitDiagnosticFormAjax(form);
        } else {
            form.reportValidity();
        }
    });
...
async function submitDiagnosticFormAjax(form) {
    const button = document.getElementById("submitDiagnosisBtn");
    const globalValidationMessage = document.getElementById("formValidationMessage");
    ...
    try {
        ...
        if (result.success) {
            ...
            setTimeout(() => {
                window.location.href = "historico_diagnosticos.php?id=" + id;
            }, 3000);
        } else {
            globalValidationMessage.textContent = result.message;
        }
    } catch (error) {
        globalValidationMessage.textContent = "Error al guardar el diagnóstico.";
