...
$result_max = mysqli_query($conex, "SELECT MAX(id_intervencion) AS max_id FROM intervenciones"); 
...
$tipo_intervencion = get_post_value($conex, 'tipo_intervencion');
$resultados_esperados = get_post_value($conex, 'resultados_esperados');
$estado = get_post_value($conex, 'estado');

// AÑADIDO: Capturar el NÚMERO DE INTERVENCION
$numero_intervencion = get_post_value($conex, 'numero_intervencion');
...
if ($beneficiario_id_str === 'NULL' || $fecha_implementacion === 'NULL' || 
    $tipo_intervencion === 'NULL' || $estado === 'NULL' || $resultados_esperados === 'NULL' || $numero_intervencion === 'NULL') {
    ...
    if ($tipo_intervencion === 'NULL') $missing_fields[] = 'tipo_intervencion';
    if ($estado === 'NULL') $missing_fields[] = 'estado';
    if ($resultados_esperados === 'NULL') $missing_fields[] = 'resultados_esperados';
    if ($numero_intervencion === 'NULL') $missing_fields[] = 'numero_intervencion'; // Validación del nuevo campo
...
$query = "INSERT INTO intervenciones (
    id_intervencion, beneficiario_id, profesional_id, fecha_implementacion, tipo_intervencion, estado,
    resultados_esperados, observaciones, numero_intervencion
) VALUES (
    $id_intervencion, $beneficiario_id_str, $profesional_sql_value, $fecha_implementacion, $tipo_intervencion, $estado,
    $resultados_esperados, $observaciones, $numero_intervencion
)";
...
if ($resultado_db) {
    // Éxito: devuelve JSON
    echo json_encode(['success' => true, 'message' => 'Intervención registrada con éxito.']);
} else {
    // Error: devuelve JSON con el mensaje específico
    $error_msg = mysqli_error($conex);
    return_error($conex, 'Error al registrar la intervención: ' . $error_msg);
}
