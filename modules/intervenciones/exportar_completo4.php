...
SELECT i.id_intervencion,
       ...
       i.numero_intervencion,
       i.fecha_implementacion,
       i.estado,
       i.tipo_intervencion,
       i.resultados_esperados,
       i.observaciones
FROM intervenciones i
JOIN beneficiarios b ON i.beneficiario_id = b.id_beneficiario
LEFT JOIN profesionales p ON i.profesional_id = p.id_profesional
";

// Filtro por beneficiario si aplica
if($beneficiario_id > 0){
    $query .= " WHERE i.beneficiario_id = $beneficiario_id";
}

// Orden
$query .= " ORDER BY i.id_intervencion ASC";
...
header('Content-Disposition: attachment;filename="intervenciones_completo_'.date('Y-m-d').'.xlsx"');
